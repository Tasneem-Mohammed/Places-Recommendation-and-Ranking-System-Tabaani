...
from core_logic.research_agents.base import BaseResearchAgent
from app.services.llm import LLMClient

load_dotenv() # Load environment variables from .env

class RestaurantSearchAgent(BaseResearchAgent):
    """
    ...
    def __init__(
        ...
        Args:
            ...
            llm_client (LLMClient, optional): Injected LLM client for message-based extraction.
        ...
        # Optional LangChain LLM; this is not strictly required for extraction
        try:
            self.llm = ChatGroq(
    ...
    async def search_websites_async(self, query: str, number_of_links: int) -> List[str]:
        ...
        """
        self.log_info(f"Searching for websites with query: {query}")
        params = {
            "engine": "google",
        ...
        try:
            ...
            self.log_info(f"Found {len(website_urls)} URLs")
            return website_urls
        except Exception as e:
            self.log_error(f"Error during SerpApi search: {e}")
            return []
    ...
    def extract_json_section(self, result_str: str) -> Optional[Dict[str, Any]]:
        ...
        if json_start == -1:
            self.log_info("No JSON object found in the response")
            return None
        brace_count = 1
        pos = json_start + 1
    ...
    async def extract_content_with_playwright_async(self, url: str) -> tuple[str, str]:
        ...
        """
        self.log_info(f"Extracting content from {url}")
        async with async_playwright() as p:
            ...
            try:
                ...
                await browser.close()
                self.log_info(f"Extracted {len(content)} characters from {url}")
    ...
    async def _extract_restaurant_names(self, text: str, query: str) -> List[str]:
        ...
        """
        self.log_info(f"Using LLM to extract restaurant names for query: '{query}'")
        prompt = f"""
        ...
        self.log_info(f"LLM raw response for restaurant extraction: {response}")
        ...
        try:
            ...
            if json_match:
                ...
                if not isinstance(extracted_names, list):
                    self.log_info(f"LLM did not return a list within JSON: {extracted_names}")
                    return []
                ...
                self.log_info("No valid JSON array found in LLM response for restaurant names.")
                return []
        except json.JSONDecodeError as e:
            self.log_error(f"Error decoding JSON from LLM response: {e}. Raw response: {response}")
            return []
        except Exception as e:
            self.log_error(f"An unexpected error occurred during restaurant name extraction: {e}")
            return []
    ...
    def postprocess_results(self, results: List[List[str]]) -> List[str]:
        ...
        """
        self.log_info(f"Postprocessing {len(results)} sets of restaurant names.")
        all_restaurants = []
        for i, res_list in enumerate(results): # Added index for debugging clarity
            self.log_info(f"Postprocessing iteration {i}: Type of res_list: {type(res_list).__name__}")
            if isinstance(res_list, list): # Ensure it's a list before extending
                all_restaurants.extend(res_list)
            else:
                self.log_info(f"WARNING: Expected a list of strings for res_list, but got {type(res_list).__name__}. Skipping this entry.")

        # Remove duplicates and sort
        unique_restaurants = sorted(list(set(all_restaurants)))
        self.log_info(f"Found {len(unique_restaurants)} unique restaurant names after postprocessing.")
    ...
    async def _process_single_url(self, url: str, query: str) -> Optional[List[str]]:
        ...
        try:
            ...
            if len(content) > 100: # Ensure enough content to process for the LLM
                ...
                if restaurant_names:
                    self.log_info(f"Extracted {len(restaurant_names)} names from {url}")
                    return restaurant_names
            else:
                self.log_info(f"Skipping {url} due to insufficient content.")
        except Exception as e:
    ...
    async def execute(self, query: str, max_urls_to_process: int = 5) -> str:
        ...
        """
        self.log_info(f"Executing restaurant search for region: {query}")
        preprocessed_query = self.preprocess_query(query)

        self.log_info(f"Initiating web search for '{preprocessed_query}'")
        urls = await self.search_websites_async(preprocessed_query, max_urls_to_process)
        self.log_info(f"Found {len(urls)} URLs for processing.")
        ...
        self.log_info(f"asyncio.gather completed. Processed {len(processed_results)} URLs.")

        extracted_restaurant_lists = []
        for i, res in enumerate(processed_results):
            if isinstance(res, Exception):
                self.log_error(f"Error in processing URL at index {i}: {res}")
        ...
        self.log_info(f"Collected {len(extracted_restaurant_lists)} lists of restaurants before final postprocessing.")
        ...
        self.log_info("Restaurant search completed.")
        return final_answer